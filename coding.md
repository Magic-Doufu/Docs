...
這種資源只需要一個開/關狀態，因此可以根據需要啟用或禁用它。由於`pson`型別包含多種型別，我們可以認為`pson`輸入函數的參數類似於布林值。 因此，在`setup()`內，您可以放置​​一個名為`led`（您也可以使用任何其他名稱）的資源，使用`<<`作為運算子，然後使用`pson`作為輸入參數型別。 該範例將使用`in`參數作為條件，以三元運算子來打開/關閉數位引腳10。
